...
app.post("/register/business", async (req, res) => {
  ...
  db.query(
    `SELECT * FROM userB WHERE b_email = ?`,
    [b_email],
    async (err, result) => {
      if (err) {
      ...
      try {
        ...
        db.query(
          sql,
          [b_name, b_phone_number, b_email, hashedPassword, b_location],
          (err, result) => {
            if (err) {
...
app.post("/login/business", (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      ...
      const token = jwt.sign({ id: user.userB_id }, process.env.JWT_SECRET, {
        expiresIn: "1h",
      });
...
app.post("/create/product", authenticateToken, (req, res) => {
  const { product_image, product_name, product_discount_price, product_price } =
    req.body;
  ...
  db.query(
    ...
    (err, result) => {
      if (err)
        return (
          res.status(500).json({ error: "Database error" }), console.log(err)
        );
...
app.get("/herbs", authenticateToken, (req, res) => {
  const sql = `SELECT * FROM products WHERE category = ?`;
...
app.get("/fruits", authenticateToken, (req, res) => {
  const sql = `SELECT * FROM products WHERE category = ?`;
...
app.get("/vegetable", authenticateToken, (req, res) => {
  const sql = `SELECT * FROM products WHERE category = ?`;
...
const isAdmin = (req, res, next) => {
  ...
  db.query(
    `SELECT role FROM userP WHERE user_id = ?`,
    [userid],
    (err, result) => {
      ...
      if (role !== "admin") {
        return res
          .status(403)
          .json({ message: "Unauthorized, only for admin" });
      }
